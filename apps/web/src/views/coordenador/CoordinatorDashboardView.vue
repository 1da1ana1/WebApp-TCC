        ...
        <main class="tab-content">
          ...
          <div v-if="currentTab === 'calendar'" class="view-calendar">
            ...
            <div class="calendar-grid">
              <div class="date-group" v-for="(item, index) in cronogramaItems" :key="item.label">
...
const limparDatas = () => {
  ...
  Swal.fire({
    icon: 'success',
    title: 'Datas limpas',
    text: 'Todas as datas foram removidas.',
    timer: 2000,
...
const editarDatas = () => {
  Swal.fire({
    ...
    text: 'Você pode editar as datas diretamente nos campos acima.',
...
const confirmarDatas = () => {
  ...
  const todasPreenchidas = cronogramaItems.value.every(item => item.startDate && item.endDate)
  
  if (!todasPreenchidas) {
    Swal.fire({
      icon: 'warning',
      title: 'Atenção',
      text: 'Por favor, preencha todas as datas antes de confirmar.',
      confirmButtonColor: 'var(--color-brand-primary)'
    })
    return
  }

  // Validar se data inicial é menor que data final
  const datasValidas = cronogramaItems.value.every(item => {
  ...
  if (!datasValidas) {
    Swal.fire({
      icon: 'error',
      title: 'Erro',
      text: 'A data inicial deve ser anterior à data final em todos os períodos.',
      confirmButtonColor: 'var(--color-brand-primary)'
    })
    return
  }
  ...
  Swal.fire({
    ...
    text: 'As datas foram salvas com sucesso.',
    confirmButtonColor: 'var(--color-brand-primary)'
  })
  
  console.log('Cronograma confirmado:', cronogramaItems.value)
}
</script>
